...
public struct RockSaltCarvingView: View {
    ...
    @State private var deposits: [HarvestableDeposit] = [
        HarvestableDeposit(type: .spire, position: CGPoint(x: 110, y: 110)),
        HarvestableDeposit(type: .cubic, position: CGPoint(x: 290, y: 165)),
        HarvestableDeposit(type: .geode, position: CGPoint(x: 190, y: 295)),
        HarvestableDeposit(type: .twin, position: CGPoint(x: 105, y: 435)),
        HarvestableDeposit(type: .block, position: CGPoint(x: 285, y: 500))
    ]
    ...
    // Target tas di HUD atas
    @State private var bagPosition = CGPoint(x: 196, y: -40)
    ...
    public var body: some View {
        GeometryReader { proxy in
            ZStack {
                ...
                VStack(spacing: 0) {
                    // PANEL ATAS: UI Quest & Kantong Khusus 5 Kristal
                    topQuestPanel
                        .zIndex(2)
                    
                    // PANEL BAWAH: Dinding Tebing Gunung Garam dengan 5 Kristal Garam
                    GeometryReader { cliffProxy in
                        cliffWallContent(size: cliffProxy.size)
                            .onAppear {
                                updatePositions(for: cliffProxy.size)
                            }
                    }
    ...
    private func updatePositions(for cliffSize: CGSize) {
        let w = cliffSize.width > 0 ? cliffSize.width : 393
        let h = cliffSize.height > 0 ? cliffSize.height : 600
        bagPosition = CGPoint(x: w * 0.5, y: -30)
        
        let targetPositions: [CGPoint] = [
            CGPoint(x: w * 0.28, y: h * 0.16),
            CGPoint(x: w * 0.74, y: h * 0.25),
            CGPoint(x: w * 0.48, y: h * 0.46),
            CGPoint(x: w * 0.26, y: h * 0.68),
            CGPoint(x: w * 0.72, y: h * 0.78)
        ]
        for i in 0..<min(deposits.count, targetPositions.count) {
            deposits[i].position = targetPositions[i]
        }
    }
    
    // MARK: - Top Quest Panel
    ...
    private var topQuestPanel: some View {
        VStack(spacing: 8) {
            ...
            // Kantong Ibu Anneth - 5 Slot Kristal Garam Berjajar
            HStack(spacing: 6) {
                ForEach(deposits) { deposit in
                    compactCrystalSlot(deposit: deposit)
                }
            }
            .padding(8)
            ...
            HStack(spacing: 12) {
                Label {
                    Text("Beliung Baja · 3x Pukulan/Kristal")
    ...
    private func compactCrystalSlot(deposit: HarvestableDeposit) -> some View {
        VStack(spacing: 2) {
            ZStack {
                RoundedRectangle(cornerRadius: 7)
                    .fill(deposit.isHarvested ? MineralDepositType.unifiedSaltColor.opacity(0.18) : Color.black.opacity(0.35))
                    .frame(height: 40)
                    .overlay(
                        RoundedRectangle(cornerRadius: 7)
                            .stroke(deposit.isHarvested ? MineralDepositType.unifiedSaltColor.opacity(0.8) : Color.white.opacity(0.1), lineWidth: 1.2)
                    )
                
                CrystalIconView(type: deposit.type, isCollected: deposit.isHarvested, size: 24)
                ...
                .opacity(deposit.isHarvested ? 1.0 : 0.0)
            }
            .frame(height: 40)
            
            Text(deposit.type.title)
                .font(.system(size: 8, weight: .bold))
                .foregroundColor(deposit.isHarvested ? .white : Color.white.opacity(0.45))
                .lineLimit(1)
    ...
    // MARK: - Cliff Wall Content
    
    private func cliffWallContent(size: CGSize) -> some View {
        ZStack {
...
// MARK: - Previews
#Preview("Rock Salt Mining - Mountain Wall") {
    RockSaltMiningView()
}
